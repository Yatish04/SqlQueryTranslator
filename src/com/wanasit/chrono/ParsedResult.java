...
public class ParsedResult implements Comparable<ParsedResult>, Cloneable{
    ...
    protected static final SimpleDateFormat PRINT_FORMAT = new SimpleDateFormat("yyyy-MM-dd"/*HH:mm"*/);
    
    @Override

   /* public void OUTPUT()
    {
        System.out.println("ParsedResult: \"" + this.text + "\" > "+ PRINT_FORMAT.format(this.start.date()) +
                ((this.end != null)? (" - " +PRINT_FORMAT.format(this.end.date())) : ""));
    }*/
    public String toString() {
        return  PRINT_FORMAT.format(this.start.date()) +
                ((this.end != null)? (" - " +PRINT_FORMAT.format(this.end.date())) : "");
    }
